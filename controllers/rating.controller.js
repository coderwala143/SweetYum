...
module.exports.addRating = async(req, res) => {
    ...
    let isAlreadyRate = product.reviews.find((review) => review.userId.equals(userId))
    if(isAlreadyRate){
    ...
    if(!hasPurchased){
        return res.status(404).json({message: "You have not purchased this product"})
    }
    ...
    const ratingsSum = product.reviews.reduce((sum, item) => sum + item.rating, 0)
